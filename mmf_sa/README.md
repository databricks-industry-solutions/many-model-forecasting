...
MMF supports post-forecast hierarchical reconciliation via three public functions in `mmf_sa/__init__.py`. Install the optional dependency first:
...
Aggregates leaf-level `train_data` into all hierarchy levels, overwrites `train_data` with all levels, and saves the summing matrix and level tags. Use this when your raw data has one row per leaf series (e.g., store-level sales) and you want MMF to forecast at all levels.
...
run_aggregation(
    spark=spark,
    train_table=f"{catalog}.{schema}.{use_case}_train_data",   # leaf-level input, overwritten with all levels
    hierarchy_s_table=f"{catalog}.{schema}.{use_case}_hierarchy_S",
    hierarchy_tags_table=f"{catalog}.{schema}.{use_case}_hierarchy_tags",
    hierarchy_cols=["country", "region", "store"],              # ordered broadest → narrowest
)
```
...
| `train_table` | `str` | Fully-qualified Delta table name. Must have `unique_id`, `ds`, `y` columns plus the hierarchy columns. Overwritten in place. |
| `hierarchy_s_table` | `str` | Output: summing matrix S (Delta table). |
| `hierarchy_tags_table` | `str` | Output: level membership tags (Delta table). |
| `hierarchy_cols` | `List[str]` | Ordered list of hierarchy columns, from broadest to narrowest (e.g. `["country", "region", "store"]`). |

---

### `derive_hierarchy_from_unique_ids()`

Derives the summing matrix and level tags from `unique_id` values that already encode hierarchy using `/` as a separator (e.g. `USA/California/Store1`). Use this when your data is already aggregated at all levels and you just need the metadata tables for reconciliation.
...
|-----------|------|-------------|
| `spark` | `SparkSession` | Active Spark session |
| `train_table` | `str` | Fully-qualified Delta table with `unique_id` values using `/` separators. Read-only — not modified. |
| `hierarchy_s_table` | `str` | Output: summing matrix S (Delta table). |
| `hierarchy_tags_table` | `str` | Output: level membership tags (Delta table). |
...
Reconciles forecasts to make them coherent across all hierarchy levels. Uses out-of-sample backtest residuals from `evaluation_output` to estimate forecast error covariances (for MinTrace/ERM). Runs entirely on the driver using Polars — no Spark workers or GPU needed.
...
|-----------|------|-------------|
| `spark` | `SparkSession` | Active Spark session |
| `best_models_table` | `str` | Forecast table with `(unique_id, ds, y)` columns. Typically the output of Skill 5 (`_best_models`), but any forecast table with these columns works. |
| `evaluation_output_table` | `str` | Backtest residuals table from `run_forecast()`. Used by MinTrace and ERM to estimate forecast error covariances. |
| `hierarchy_s_table` | `str` | Summing matrix created by `run_aggregation()` or `derive_hierarchy_from_unique_ids()`. |
| `hierarchy_tags_table` | `str` | Level membership tags table. |
| `reconciliation_output` | `str` | Output Delta table for reconciled forecasts. |
| `freq` | `str` | Frequency code: `H`, `D`, `W`, or `M`. |
| `date_col` | `str` | Date column name in the forecast table (default: `ds`). |
| `target` | `str` | Target column name in the forecast table (default: `y`). |
| `method` | `str` | Reconciliation method: `BottomUp`, `TopDown`, `MiddleOut`, `MinTrace` (default), or `ERM`. |

**Output table schema:**

| Column | Type | Description |
|--------|------|-------------|
| `unique_id` | string | Series identifier (encodes hierarchy level via `/`) |
...
| `hierarchy_level` | string | Hierarchy level name (e.g. `country`, `region`, `store`) |
...
|--------|-------------|-------------|
| `MinTrace` | Minimizes forecast error variance by estimating cross-level covariances from backtest residuals | **Recommended** — statistically optimal when backtest data is available |
| `BottomUp` | Sums leaf-level forecasts upward | Simple, no residuals needed; loses top-level information |
| `TopDown` | Distributes top-level forecast downward using historical proportions | When top-level forecast is most reliable |
| `MiddleOut` | Anchors at an intermediate level | When a specific level is most accurate |
| `ERM` | Learns an optimal reconciliation matrix from residuals | Experimental; requires sufficient backtest history |
